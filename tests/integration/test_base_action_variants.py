"""Integration tests for template-based variant generation.

Tests verify that:
- Variant templates are created in actions/ directory
- Templates are rendered with persona/tool/library context
- Template-based workflow operates correctly
"""
...
def temp_project_dir(tmp_path):
    ...
    # Create variant instruction templates for on-demand generation
...
def test_variant_generated_from_action_template(temp_project_dir, temp_output_dir):
    """Test that variant prompts are generated from variant action templates."""
...
def test_variant_generated_on_demand_with_cli(temp_project_dir, temp_output_dir):
    """Test that variant action template is generated on-demand when missing."""
    # No update-research.md.j2 created initially
    # Variant instruction template exists (created in fixture)
    ...
    # Mock CLI tool to generate variant action template
    mock_tool = Mock()
    mock_tool.name = "mock_tool"
    mock_tool.is_available.return_value = True
    mock_tool.generate_variant.return_value = (
        "{{ persona }}\n\nAI-generated update action template."
    )
    ...
    # Verify variant action template was created
    variant_template_path = temp_project_dir / "actions" / "update-research.md.j2"
    assert variant_template_path.exists()
    assert "{{ persona }}" in variant_template_path.read_text()

    # Verify variant prompt was generated from the template
    variant_file = temp_output_dir / "update-research.prompt.md"
    assert variant_file.exists()


def test_mixed_direct_and_on_demand_variants(temp_project_dir, temp_output_dir):
    """Test mixed scenario with some direct templates and some on-demand generation."""
    ...
    # Don't create action/refine-research.md.j2 (will be generated on-demand)
    ...
    # Mock CLI tool for on-demand generation (refine only)
    mock_tool = Mock()
    mock_tool.name = "mock_tool"
    mock_tool.is_available.return_value = True
    mock_tool.generate_variant.return_value = (
        "{{ persona }}\n\nAI-generated refine template."
    )
    ...
    # Verify update was generated from existing template
    ...
    # Verify refine action template was created on-demand
    refine_template = temp_project_dir / "actions" / "refine-research.md.j2"
    assert refine_template.exists()

    # Verify refine prompt was generated
    refine_file = temp_output_dir / "refine-research.prompt.md"
    assert refine_file.exists()

    # Verify CLI was called only once (for refine template generation, not update)
...
def test_generate_all_with_mixed_variants(temp_project_dir, temp_output_dir):
    """Test generate_all with mixed direct and on-demand variant generation."""
    ...
    # Mock CLI tool for on-demand refine template generation
    mock_tool = Mock()
    mock_tool.name = "mock_tool"
    mock_tool.is_available.return_value = True
    mock_tool.generate_variant.return_value = (
        "{{ persona }}\n\nOn-demand refine template."
    )
    ...
    # - update-research.prompt.md (variant from existing template)
    # - refine-research.prompt.md (variant from on-demand template)
    ...
    # Verify update came from existing template
    update_content = (temp_output_dir / "update-research.prompt.md").read_text()
    assert "Update variant" in update_content

    # Verify refine template was created on-demand
    refine_template = temp_project_dir / "actions" / "refine-research.md.j2"
    assert refine_template.exists()

    # Verify CLI was called only once (for refine template, not update)
    assert mock_tool.generate_variant.call_count == 1
...
def test_on_demand_generation_continues_on_cli_tool_error(
    temp_project_dir, temp_output_dir
):
    """Test generation continues when on-demand template generation fails."""
    # Create one direct template and one that will fail on-demand generation
    action_dir = temp_project_dir / "actions"
    (action_dir / "update-research.md.j2").write_text(
        "{{ persona }}\n\nUpdate variant."
    )
    ...
    prompt_config = PromptConfig(
        persona="researcher",
        action="research",
        variants=["update", "refine"],  # refine will fail on-demand generation
        cli_tool=None,
    )
    ...
    # Should have 2 files: base + update (refine failed on-demand generation)
    assert len(result.files_generated) == 2

    # Update exists (from existing template)
    assert (temp_output_dir / "update-research.prompt.md").exists()

    # Refine doesn't exist (on-demand generation failed)
...
def test_logging_indicates_generation_strategy(
    ...
    # Mock CLI tool for on-demand generation
    mock_tool = Mock()
    mock_tool.name = "mock_tool"
    mock_tool.is_available.return_value = True
    mock_tool.generate_variant.return_value = (
    ...
    # Should have log for on-demand template generation
    assert any(
        "on-demand" in msg for msg in log_messages
    ), "Missing log for on-demand generation"
